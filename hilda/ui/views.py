...
class StackView(View):
    ...
    def __str__(self) -> str:
        ...
        for k, v in stack_mapping.items():
            ...
            fmt = (
                f"{style(hex(k), fg=self.color_scheme.address)}|+"
...
class DisassemblyView(View):
    ...
    def __init__(
        self, hilda_client, color_scheme, instruction_count=DEFAULT_INSTRUCTION_COUNT, flavor=DEFAULT_DISASSEMBLY_FLAVOR
    ):
        """
        :param hilda_client: hilda.hilda_client.HildaClient
...
class BackTraceView(View):
    ...
    def __str__(self) -> str:
        ...
        bt_colored = []
        for pair in bt:
            bt_colored.append([style(pair[0], fg=self.color_scheme.address), pair[1]])

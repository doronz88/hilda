...
class NanoV2Arena:
    ...
    size_per_slot: ClassVar = [16, 32, 48, 64, 80, 96, 112, 128, 144, 160, 176, 192, 208, 224, 240, 256]
    blocks_per_size: ClassVar = [2, 10, 11, 10, 5, 3, 3, 4, 3, 2, 2, 2, 2, 2, 1, 2]
    slots_per_size: ClassVar = [1024, 512, 341, 256, 204, 170, 146, 128, 113, 102, 93, 85, 78, 73, 68, 64]
    ...
    def __init__(self, arena_base_addr):
        self.nanov2_arena_struct = Struct(
            ...
            *[
                f"blocks_{idx}"
                / Array(
                    self.blocks_per_size[idx],
                    Struct("content" / Array(self.slots_per_size[idx], Struct("Q" / Bytes(self.size_per_slot[idx])))),
                )
    ...
    def dump_arena_metadata_block(self):
        ...
        for meta_block in self.arena_metadata_block.arena_block_meta:
            ...
            if block_meta_struct.in_use:
                print(
                    f"Arena block index {block_idx} (CPU {int(block_idx / 64) - 1}): next: {block_meta_struct.next_slot}"
                )
            block_idx = block_idx + 1


RACK_TYPE = Enum(Int32ul, RACK_TYPE_NONE=0, RACK_TYPE_TINY=1, RACK_TYPE_SMALL=2, RACK_TYPE_MEDIUM=3)
